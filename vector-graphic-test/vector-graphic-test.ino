...
void drawVectorGraphic(const uint8_t *graphic, byte x, byte y, byte scaleFactor) {
    ...
    byte graphicSize = 128;

    for (byte i = 0; i < numRows ; i++) {
        // byte bite = ++graphic;
        // arduboy.print(bite);

        byte imgCtr = (graphicSize / scaleFactor) / 2;

         int x0 = (pgm_read_byte(++graphic) / scaleFactor + x) - imgCtr,
...
void loop(void) {
  ...
  // switch (screenState) {
    ...
    // case 1:
      ...
      drawVectorGraphic(fighter1, fighterX, fighterY, distance);
